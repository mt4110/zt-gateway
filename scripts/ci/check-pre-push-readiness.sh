...
echo "[1/6] go test ./gateway/zt"
go test ./gateway/zt -count=1

echo "[2/6] zt contract gate"
bash ./scripts/ci/check-zt-contract-gate.sh

echo "[3/6] go test ./tools/secure-pack/internal/workflows"
go test ./tools/secure-pack/internal/workflows -count=1

echo "[4/6] fixture supply-chain gate"
bash ./scripts/ci/check-zt-setup-json-gate.sh

echo "[5/6] actual repo supply-chain gate"
...
echo "[6/6] git status summary (manual review before commit/push)"
git status --short
